...
$sql_reply = "SELECT * FROM deal_list where uid ='$uid' and state = '2' ORDER BY created DESC LIMIT $start_point,$list";
...
while($fetch = mysqli_fetch_array($reply_data)){
    ...
    array_push($resultArray,array('priceAvg'=>$priceAvg, 'name'=>$name, 'num'=>$num, 'price'=>$price, 'total'=>$total, 'created'=>$filtered_created, 'state'=>$state, 'idx'=>$idx));
}
